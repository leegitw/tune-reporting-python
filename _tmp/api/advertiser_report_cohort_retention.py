...
class AdvertiserReportCohortRetention(AdvertiserReportCohortBase):
    ...
    def count(self,
        ...
        if "retention_measure" in map_params and map_params["retention_measure"] is not None:
            map_query_string = self._validate_retention_measure(map_params, map_query_string)
    ...
    ## Validate query string fields parameter pertaining to retention measure.
    #  @param dict map_params
    #  @param dict map_query_string
    #  @return dict map_query_string
    #  @throws ValueError
    @staticmethod
    def _validate_retention_measure(map_params, map_query_string):
        """Validate 'retention_measure' parameter.
        ...
        if 'retention_measure' not in map_params:
            raise ValueError("Parameter 'retention_measure' is not defined.")

        retention_measure = map_params['retention_measure']
        ...
        if retention_measure is None or not retention_measure:
            raise ValueError("Parameter 'retention_measure' is not defined.")
        if (not isinstance(retention_measure, str) or
                (retention_measure not in retention_measures)):
            raise TuneSdkException(
                "Parameter 'retention_measure' is invalid: '{}'.".format(
                    retention_measure
                )
            )

        if "fields" in map_query_string:
            map_query_string["fields"] = map_query_string["fields"] + "," + retention_measure
        else:
            map_query_string["fields"] = retention_measure
