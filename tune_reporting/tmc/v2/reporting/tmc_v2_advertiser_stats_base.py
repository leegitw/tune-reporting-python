...
class TuneV2AdvertiserStatsBase(TuneMobileAppTrackingApi):
    ...
    def collect(
        ...
        """
        self.logger.info(
            "TMC v2 Advertiser Stats: Collect: {}".format(
                request_action
            )
        )
        ...
        if not auth_value:
            raise ValueError(
                error_message="TMC v2 Advertiser Stats: Collect: Value 'auth_value' not provided."
            )
        if not auth_type or \
                not TuneV2AuthenticationTypes.validate(auth_type):
            raise ValueError(
                error_message="TMC v2 Advertiser Stats: Collect: Value 'auth_type' not valid."
            )
        if not auth_type_use or \
                not TuneV2AuthenticationTypes.validate(auth_type_use):
            raise ValueError(
                error_message="TMC v2 Advertiser Stats: Collect: Value 'auth_type_use' not valid."
            )
    ...
    def stream(
        ...
        """
        self.logger.info(
            "TMC v2 Advertiser Stats: Stream"
        )

        if not auth_value:
            raise ValueError(
                error_message="TMC v2 Advertiser Stats: Stream: Value 'auth_value' not provided."
            )
        if not auth_type or \
                not TuneV2AuthenticationTypes.validate(auth_type):
            raise ValueError(
                error_message="TMC v2 Advertiser Stats: Stream: Value 'auth_type' not valid."
            )
        if not auth_type_use or \
                not TuneV2AuthenticationTypes.validate(auth_type_use):
            raise ValueError(
                error_message="TMC v2 Advertiser Stats: Stream: Value 'auth_type_use' not valid."
            )
    ...
    def _find_v2(
        ...
        try:
            ...
            self.logger.error(
                "TMC v2 Advertiser Stats: Failed: {}".format(
                    str(tmv_ex)
                )
            )
            raise
            ...
            self.logger.error(
                "TMC v2 Advertiser Stats: {}".format(
    ...
    def _export_v2_download_csv(
        ...
        if not request_params:
            raise ValueError(
                "Missing parameter 'request_params'"
            )
    ...
    def _export_stream_v2(
        ...
        self.logger.debug(
            (
                "TMC v2 Advertiser Stats: Gather Export by Streaming: "
                "Actions '{}' and '{}'"
            ).format(
                export_action,
                export_status_action
    ...
    def _export_v2_job_to_queue(
        ...
        if export_action == TuneV2AdvertiserStatsExportActions.EXPORT:
            if ('job_id' not in json_data or
                    not json_data['job_id']):
                raise TuneReportingError(
                    error_message=(
                        "TMC v2 Advertiser Stats: "
                        "Action '{}': "
                        "Response missing 'export_job_id': {}"
                    ).format(
                        export_action,
                        str(json_data)
                    )
    ...
    def _check_v2_job_status_on_queue(
        ...
        while True:
            try:
                ...
                self.logger.error(
                    "TMC v2 Advertiser Stats: Check Job Status: Failed: {}".format(
                        str(tmv_ex)
                    )
                )
                raise
            ...
            if not http_status_successful:
                raise TuneReportingError(
                    error_message=(
                        "Failed to get export status on queue: {}"
                    ).format(
    ...
    def _read_stats_data(
        ...
        self.logger.info(
            "TMC v2 Advertiser Stats: Reading",
            extra={
                'report_url': report_url,
        ...
        if not report_reader:
            raise TuneReportingError(
                error_message=(
                    "Report reader not created for format {}"
                ).format(
                    report_format
                )
            )
