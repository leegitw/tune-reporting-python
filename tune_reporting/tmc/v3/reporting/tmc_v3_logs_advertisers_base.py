...
from tune_reporting.readers.report_reader_json import (
    ReportReaderJSON
)
from tune_reporting import (
    __python_required_version__
)
from tune_reporting.readers.report_reader_csv import (
    ReportReaderCSV
)
...
class TuneV3LogsAdvertisersBase(TuneMobileAppTrackingApi):
    ...
    def collect(
        ...
        if not auth_value:
            raise ValueError(
                "TMC v3 Logs Advertisers Base: Collect: Value 'auth_value' not provided."
            )
        if not auth_type:
            raise ValueError(
        ...
        if not advertiser_id:
            raise ValueError(
                "TMC v3 Logs Advertisers Base: Collect: Value 'advertiser_id' not valid."
            )
        ...
        if auth_type == TuneV2AuthenticationTypes.API_KEY:
            ...
            raise ValueError(
                error_message="Invalid 'auth_type': '{}'".format(
                    auth_type
                )
            )
        ...
        self.controller = self._CONTROLLER.format(
    ...
    def _find_v3(
        ...
        request_params,
        request_retry=None,
        request_label="TMC v3 Logs Advertisers"
    ):
        ...
        try:
            ...
            self.logger.error(
                "TMC v3 Logs Advertisers Base: Failed: {}".format(
                    str(tmv_ex)
                )
            )
            raise
            ...
            self.logger.error(
                "TMC v3 Logs Advertisers Base: {}".format(
    ...
    def _export_v3_download_csv(
        ...
        while not done:
            ...
            export_job = self._export_v3_job_to_queue(
                request_params,
                request_retry
            )
        ...
        for export_job in export_jobs:
            ...
            report_url = self._check_v3_job_status_on_queue(
                export_job_handle,
                request_retry=request_retry
            )

            generator_data_stats = self._read_stats_data(
                export_job_handle,
                report_url
            )
    ...
    def _export_v3_job_to_queue(
        ...
        request_params,
        request_retry=None,
        request_label="TMC v3 Job On Queue"
    ):
        ...
        json_response = self.request_mv_integration.validate_json_response(
            response,
            request_label="TMC v3 Logs Advertisers: '{}': Action 'exports'".format(
                self.logs_advertisers_type
            )
        )

        if ('handle' not in json_response or
                not json_response['handle']):
            raise TuneReportingError(
                error_message=(
                    "TMC v3 Logs Advertisers Base: "
                    "Logs '{}': "
    ...
    def _check_v3_job_status_on_queue(
        self,
        export_job,
        request_retry=None,
        request_label="TMC v3 Job Status On Queue"
    ):
        ...
        while True:
            ...
            if not http_status_successful:
                raise TuneReportingError(
                    error_message="Failed to get export status on queue: {}".format(
            ...
            if (export_percent_complete == 100 and
                    json_response["status"] == "complete" and
                    json_response["url"]):
    ...
    def _read_stats_data(
        ...
        if not export_job_handle:
            raise ValueError(
                "Argument 'export_job_handle' not assigned."
            )
        if not report_url:
            raise ValueError(
                "Argument 'report_url' not assigned."
            )
        ...
        if report_format == TuneV3LogsAdvertisersResponseFormats.JSON:
            ...
            raise TuneReportingError(
                error_message="Unexpected report format: '{}'".format(
                    report_format
                )
            )
        ...
        if not report_reader:
            raise TuneReportingError(
                error_message="Report reader not created for format {}".format(
                    report_format
                )
            )
    ...
    def tune_v3_request_retry_func(
        self,
        response
    ):
        ...
        self.logger.debug(
            "Check for Retry: Response: Status: {}, Errors: '{}'".format(
                tune_v2_errors_error_code,
                tune_v2_errors_message
            )
        )
